...
/// 城西路
#define SRC "../../data/videos/test_5.mp4"
int roiX = 195 * 2;
int roiY = 258 * 2;
int roiWidth = 384 * 2;
int roiHeight = 99 * 2;
...
void onGaussianChange(int _x, void* _ptr) {
    ...
    float *xs = new float[gaussianSize];
    float *ys = new float[gaussianSize];
...
vector<int> lanePF(Mat& _iInput) {
    ...
    float *stdLine = new float[totalLen];
...
int main()
{
    ...
    VideoWriter oVideoWriter ("LaneDetection_test.avi", CV_FOURCC('P','I','M','1'), 30, Size(width / 1.5, height / 1.5), true);
